...
#
# author: qingehua
# 功能：将ID与微博地址绑定后爬取评论用户的前300条微博
# 输入格式：
# 1 ID | 2 微博名 （可选） | 3 微博地址
# 输出格式：
...
def LoginWeibo(username, password):
    try:
        ...
        #elem_pwd = driver.find_element_by_name("loginPassword")
        #elem_pwd.send_keys(password)  #密码
        time.sleep(30)
        print u'登陆成功...'
        
    except Exception,e:      
        print "Error: Login occurs error!",e
    finally:    
...
if __name__ == '__main__':
    ...
    for i, cmmt_usr_id in enumerate(weibo_url_col):
    	print u'第'+str(i+1)+ '/' + str(readSheet.nrows) + u'用户信息'
    	if 'https://weibo.cn/comment/hot' in cmmt_usr_id:
    			continue

    	innner_id = readSheet.row(i)[0]
    	innner_id = str(innner_id).split('\'')
    	innner_id = innner_id[1]
    	print cmmt_usr_id
    	VisitLeafPage(str(cmmt_usr_id),innner_id)

    	# 发现了firefox和geckodriver不兼容的bug，目前受版本影响还没有直接解决办法，只能先把第一行跑两遍
    	if i == 0:
    		temo_url = str(cmmt_usr_id)
    		VisitLeafPage(str(cmmt_usr_id),innner_id)
